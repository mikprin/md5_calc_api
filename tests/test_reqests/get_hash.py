...
def get_hash(id,test_url = "http://127.0.0.1:8000/", verbose = False):
    # test_file = open("test_file", "rb")
    
    request_url = os.path.join( test_url, f"gethash/?file_id={id}")

    test_response = requests.get(request_url, data={"file_id": id , "accept" : "application/json" } )
